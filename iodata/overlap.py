...
def compute_overlap(obasis: MolecularBasis, atcoords: np.ndarray) -> np.ndarray:
    ...
    # define a python ufunc (numpy function) for broadcasted calling WRT x, y, z coords
            ...
            if prefactor > 1.e-15:
                ...
                for iexp0, (a0, cc0) in enumerate(zip(shell0.exponents, shell0.coeffs[:, 0])):
                    ...
                    for iexp1, (a1, cc1) in enumerate(zip(shell1.exponents, shell1.coeffs[:, 0])):
                        ...
                        at = a0 + a1
                        a0_a1_div_at = -a0 * a1 / at
                        prefactor = np.exp(a0_a1_div_at * rij_norm_sq)
                        if prefactor < 1.0e-15:
                            continue
                        # prepare some pre-factors to save FLOPS in inner loop
                        rn = (a0_r0 + a1 * r1) / at
                        pf = np.exp(a0_a1_div_at * rij_vec_sq) * np.sqrt(np.pi / at)
                        two_at = 2 * at
                        rn_0 = rn - r0
                        rn_1 = rn - r1
                        ...
                        for s0, n0, in enumerate(iter_cart_alphabet(shell0.angmoms[0])):
                            for s1, n1, in enumerate(iter_cart_alphabet(shell1.angmoms[0])):
                                # broadcasted so that x,y,z are done simultaneously
                                v = np.prod(pf * compute_overlap_1d(rn_0, rn_1, n0, n1, two_at))
